...
class ToursController extends Controller
{
    ...
    public function deleteTour($tourId)
    {
        ...
        if(Auth::user()->company) {
             $path = $_SERVER['DOCUMENT_ROOT'].'/upload/archive/'.preg_replace('/\s+/', '', Auth::user()->company).'/tours/'.$tourId;
         } else {
            $path = $_SERVER['DOCUMENT_ROOT'].'/upload/archive/'. preg_replace('/\s+/', '', Auth::user()->name).Auth::user()->id.'/tours/'.$tourId;
         }
        ...
        if(Auth::user()->company) {
        $this->recurse_copy($_SERVER['DOCUMENT_ROOT'].'/upload/'.preg_replace('/\s+/', '', Auth::user()->company).'/tours/'.$tourId,$_SERVER['DOCUMENT_ROOT'].'/upload/archive/'.preg_replace('/\s+/', '', Auth::user()->company).'/tours/'.$tourId);
    } else {

        $this->recurse_copy($_SERVER['DOCUMENT_ROOT'].'/upload/'. preg_replace('/\s+/', '', Auth::user()->name).Auth::user()->id.'/tours/'.$tourId,$_SERVER['DOCUMENT_ROOT'].'/upload/archive/'.  preg_replace('/\s+/', '', Auth::user()->name).Auth::user()->id.'/tours/'.$tourId);
    }
